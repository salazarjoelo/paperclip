...
- a continuation cancelled as `issue_continuation_waiting_on_review` first converts open children or blockers into a first-class dependency wait; without a real target it is classified as `deliberate_wait_without_target` and gives the invokable original owner at most three normal-model disposition-repair attempts (immediate, 60 seconds, and 120 seconds, with bounded jitter)
...
- backwards-compatible upgrades count consecutive historical `issue_continuation_waiting_on_review` cancellations for the unchanged accepted-interaction source state against the same three-attempt disposition-repair ceiling; missing pre-upgrade recovery-action rows do not reset the budget
