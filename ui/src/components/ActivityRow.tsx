...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  ...
  const actorName = actor?.name ?? (event.actorType === "system" ? t('activity.actors.system') : userProfile?.label ?? (event.actorType === "user" ? t('activity.actors.user') : event.actorId || t('activity.actors.unknown')));
