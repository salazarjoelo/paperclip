...
export const DISPOSITION_REPAIR_MAX_ATTEMPTS = 3;
export const DISPOSITION_REPAIR_BASE_DELAYS_MS = [0, 60_000, 120_000] as const;
...
export function dispositionRepairDelayMs(attempt: number, fingerprint: string) {
  const baseDelayMs = DISPOSITION_REPAIR_BASE_DELAYS_MS[attempt - 1];
  if (baseDelayMs === undefined) throw new Error(`Invalid disposition repair attempt: ${attempt}`);
